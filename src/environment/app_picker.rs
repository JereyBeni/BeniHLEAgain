...
pub fn app_picker(options: Options) -> Result<(PathBuf, Vec<String>), String> {
    ...
    let apps: Result<Vec<AppInfo>, String> = if !apps_dir.is_dir() {
        Err(format!(
            "🔥 ¡Ojo bro! No existe la carpeta {}. \
             Creala e inyectale tus .ipa/.app de Talking Friends.",
            apps_dir.display()
        ))
    } else {
        enumerate_apps(&apps_dir)
            ...
            .and_then(|apps| {
                if apps.is_empty() {
                    Err(format!(
                        "💀 No hay apps ni juegos en {}. \
                         ¡Meté unos binarios ahí!",
                        apps_dir.display()
                    ))
                } else {
                    Ok(apps)
...
fn enumerate_apps(apps_dir: &Path) -> Result<Vec<AppInfo>, std::io::Error> {
    ...
    for app in std::fs::read_dir(apps_dir)? {
        ...
        let (bundle, fs) =
            match BundleData::open_any(&app_path).and_then(|bundle_data| {
                ...
                Err(e) => {
                    log!(
                        "Warning: No se pudo abrir el bundle {}: {} (skipping)",
                        app_path.display(),
                        e
                    );
...
const CLASSES: ClassExports = objc_classes! {
    ...
    - (())quickOptionsShow {
        env.objc
            .borrow_mut::<AppPickerDelegateHostObject>(this)
            .quick_options_show = true;
    }

    - (())quickOptionsHide {
        env.objc
            .borrow_mut::<AppPickerDelegateHostObject>(this)
            .quick_options_hide = true;
    }

    - (())scaleHackDefault {
        env.objc
            .borrow_mut::<AppPickerDelegateHostObject>(this)
            .scale_hack_default = true;
    }
    ...
    - (())orientationLandscapeLeft {
        env.objc
            .borrow_mut::<AppPickerDelegateHostObject>(this)
            .orientation_landscape_left = true;
    }

    - (())orientationLandscapeRight {
        env.objc
            .borrow_mut::<AppPickerDelegateHostObject>(this)
            .orientation_landscape_right = true;
    }

    - (())orientationPortraitUpsideDown {
        env.objc
            .borrow_mut::<AppPickerDelegateHostObject>(this)
            .orientation_portrait_upside_down = true;
    }

    - (())analogStickTiltControls:(id)switch {
        let switch_state: bool =
            msg![env; switch isOn];
    ...
    - (())deviceModelToggle {
        env.objc
            .borrow_mut::<AppPickerDelegateHostObject>(this)
            .device_model_toggle = true;
    }

    - (())deviceModelScrollUp {
        env.objc
            .borrow_mut::<AppPickerDelegateHostObject>(this)
            .device_model_scroll_up = true;
    }

    - (())deviceModelScrollDown {
        env.objc
            .borrow_mut::<AppPickerDelegateHostObject>(this)
            .device_model_scroll_down = true;
    }
    ...
    - (())visitWebsite {
        ...
        let url =
            ns_string::get_static_str(
                env,
                "https://github.com/JereyBeni/BeniHLEAgain",
            );

        let url: id =
            msg_class![env; NSURL URLWithString:url];

        let ui_application: id =
            msg_class![env; UIApplication sharedApplication];

        assert!(
            msg![env; ui_application openURL:url]
        );
    }
...
fn show_app_picker_gui(
    ...
    let environment =
        Environment::new_without_app(
            options,
            icon,
        )?;

    Ok(
        environment.run_app_picker(
            |env| app_picker_inner(env, apps)
        )
    )
}
...
fn app_picker_inner(
    ...
    let ui_application: id =
        msg_class![env; UIApplication new];

    let delegate =
        env.objc.get_known_class(
            "_touchHLE_AppPickerDelegate",
            &mut env.mem,
        );
    ...
    () = msg![
        env;
        ui_application setDelegate:delegate
    ];
    ...
    let window: id =
        msg_class![env; UIWindow alloc];

    let window: id =
        msg![env; window initWithFrame:bounds];


    let app_frame: CGRect =
        msg![env; screen applicationFrame];


    let main_view: id =
        msg_class![env; UIView alloc];

    let main_view: id =
        msg![
            env;
            main_view initWithFrame:app_frame
        ];
    ...
    // res/apple_earth.png
    //
    // We load it directly from the executable resources, convert it to the
    // emulator's internal CGImage representation, then create a UIImageView.
    //
    // The image view is inserted BEFORE the title and other UI elements,
    // so it remains behind everything else.
    // -------------------------------------------------------------------------
    ...
    let earth_bytes: &[u8] =
        include_bytes!(
            concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/res/res/apple_earth.png"
            )
        );

    let earth_image =
        match Image::from_bytes(earth_bytes) {
            Ok(image) => image,

            Err(_) => {
                log!(
                    "[BeniHLE] WARNING: Could not load \
                     res/res/apple_earth.png"
                );

                // Fall back to black if the image cannot be decoded.
                let bg_color: id =
                    msg_class![env; UIColor blackColor];

                () = msg![
                    env;
                    main_view setBackgroundColor:bg_color
                ];

                // Continue without the Earth image.
                ...
                let title_label: id =
                    msg_class![env; UILabel alloc];

                let title_label: id =
                    msg![
                        env;
                        title_label initWithFrame:title_frame
                    ];

                let title_text =
                    ns_string::from_rust_string(
                        env,
                        "BeniHLEAgain // App Launcher"
                            .to_string(),
                    );
                ...
                () = msg![
                    env;
                    title_label setTextColor:neon_color
                ];

                () = msg![
                    env;
                    main_view addSubview:title_label
                ];
    ...
    // Convert our internal image to a CGImage.
    let earth_cg_image =
        cg_image::from_image(
            env,
            earth_image,
    ...
    let earth_view: id =
        msg![
            env;
            earth_view initWithImage:
                earth_ui_image
        ];


    // Stretch the image across the complete App Picker.
    () = msg![
        env;
        earth_view setFrame:app_frame
    ];


    // Add the Earth FIRST so all later UI appears above it.
    () = msg![
        env;
        main_view addSubview:earth_view
    ];
    ...
    // -------------------------------------------------------------------------
    //
    // The original image already contains a black background.
    // We therefore don't add another overlay here.
    // -------------------------------------------------------------------------
    ...
    () = msg![
        env;
        title_label setTextColor:
            neon_color
    ];


    // Add title AFTER Earth so it appears above the image.
    () = msg![
        env;
        main_view addSubview:title_label
    ];
